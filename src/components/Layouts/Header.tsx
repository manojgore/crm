...
interface userAccountSettingsInterface{
...
const Header = () => {
  ...
  const [userAccountSettings, setUserAccountSettings] = useState<userAccountSettingsInterface | null>(null);
  const fetchUser = async() =>{ 
    try {
        ...
        if (response.data.success) {
            ...
            setUserAccountSettings(response.data.accountSettings[0]);

            if(response.data.results[0].number === "0"){
                    ...
                    <div className="sm:flex-1 ltr:sm:ml-0 ltr:ml-auto sm:rtl:mr-0 rtl:mr-auto flex justify-end items-center space-x-1.5 lg:space-x-2 rtl:space-x-reverse dark:text-[#d0d2d6]">
                        ...
                        <div className="dropdown shrink-0 flex">
                            <Dropdown
                                ...
                                button={<img className="w-9 h-9 rounded-full object-cover saturate-50 group-hover:saturate-100" src={userAccountSettings && userAccountSettings.image ? userAccountSettings.image : "/assets/images/user-profile.jpeg"} alt="userProfile" />}
                            >
                                        ...
                                        <div className="flex items-center px-4 py-4">
                                            ...
                                            <div className="ltr:pl-4 rtl:pr-4 truncate">
                                                <h4 className="text-base">
                                                    {user && user.username}
                                                    {user && user.subscribed === 1 && <span className="text-xs bg-success-light rounded text-success px-1 ltr:ml-2 rtl:ml-2">Active</span>}
                                                    {user && user.subscribed === 0 && <span className="text-xs bg-red-200 rounded text-red-700 px-1 ltr:ml-2 rtl:ml-2">Not Active</span>}
                                                </h4>
